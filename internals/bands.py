...
class GroundBand(Network):
    ...
    def __init__(self, context, ground, illuminee, index):
        ...
        self.screen_placement = AnimatedScreenPlacement(
            {"mesh": context["mesh"], "index": str(self.index)}
        )
        ...
        for shade_index, (facet_image, luminance_value) in enumerate(
            ...
            tracking_projection_context = context | {
                "facet": context["index"],
                "image": f"shade{shade_index}",
            }
            tracking_projection = self.build(
                TrackingProjection(
                    tracking_projection_context,
                    self.screen_placement,
                    facet_image,
    ...
    def z_gs(self, frame):
        return -(
            gcn.band_offset.get(t=frame) + gcn.band_spacing.get() * (self.index + 0.5)
        )
    ...
    def x_ss_to_ground_xyz_ws(self, x_ss, frame):
        ...
        camera_ground[1] = 0
        return tuple(
            z_gs * zuc + x_gs * xuc + cc
            for zuc, xuc, cc in zip(z_unit, x_unit, camera_ground)
        )

    def ground_xyz_ws_to_uv(self, xyz_ws, frame):
        self.ground.xyz_ws_to_uv.set(xyz_ws)
        return self.ground.u_from_xyz_ws.get(t=frame), self.ground.v_from_xyz_ws.get(
            t=frame
        )
    ...
    def animate(self):
        ...
        for frame in range(playback_min + 1, playback_max + 1):
            ...
            prev_cam_trans = self.ground.decompose_camera.outputTranslate.get(
                t=frame - 1
            )
            current_cam_trans = self.ground.decompose_camera.outputTranslate.get(
                t=frame
            )
            prev_dist = (
                sum(
                    [
                        (cam_c - band_c) ** 2
                        for cam_c, band_c in zip(prev_cam_trans, prev_xyz_ws)
                    ]
                )
                ** 0.5
            )
            current_dist = (
                sum(
                    [
                        (cam_c - band_c) ** 2
                        for cam_c, band_c in zip(current_cam_trans, current_xyz_ws)
                    ]
                )
                ** 0.5
            )
            ...
            dx_avg, dy_avg = [
                sum(p2[c] - p1[c] for p1, p2 in xy_ss_list) / len(xy_ss_list)
                for c in range(2)
            ]
            ...
            if scale_factor == 1:
                ...
                x_avg, y_avg = [
                    sum(p2[c] for _, p2 in xy_ss_list) / len(xy_ss_list)
                    for c in range(2)
                ]
...
class BandShader(Network):
    ...
    def __init__(self, context, mesh):
        ...
        original_mesh_transformer = self.utility(
            "transformGeometry", "original_mesh_transformer"
        )
        ...
        closest_point_to_flattened_ground = self.utility(
            "closestPointOnMesh", "closest_point_to_flattened_ground"
        )
        flattener.outputGeometry >> closest_point_to_flattened_ground.inMesh

        closest_point_to_regular_ground = self.utility(
            "closestPointOnMesh", "closest_point_to_regular_ground"
        )
        ...
        camera_ground_vector_calculator = self.utility(
            "normalize", "camera_ground_vector"
        )
        ...
        point_relative_to_camera = self.utility(
            "aiSubtract", "point_relative_to_camera"
        )
        ...
        normalized_depth = self.divide(
            depth_minus_offset, total_depth, "normalized_depth"
        )
    ...
    def animate(self):
        anim_curve_list = gcn.band_offset.listConnections(
            s=True, d=False, type="animCurve"
        )
        if anim_curve_list:
