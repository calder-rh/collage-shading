...
def is_valid_selection(path):
    path = Path(path)
    return is_palette(path) or (
        is_palette(path.parent) and re.fullmatch(shade_select_regex, path.name)
    )
...
class ImagesPalette(Palette):
    def __init__(self, dir):
        ...
        self.images = [
            ...
            if item.is_file() and re.fullmatch(shade_regex, item.name)
        ]
    ...
    def _default_shade_settings(self):
        ...
        for i, image_file in enumerate(self.images):
            ...
            shade_settings.append(
                {"filename prefix": f"s{shade_number}", "luminance": ratio}
            )
    ...
    def make(self, scale, edge_distance):
        ...
        for setting in settings["shades"]:
            image = [
                ...
                if image.name.startswith(setting["filename prefix"])
            ][0]
...
def indices_to_path(indices):
    ...
    for index in indices:
        ...
        if not next_path:
            raise Exception(
                f"There is no palette at a path corresponding to {indices}."
            )
...
def get_path_type(indices):
    try:
        ...
        if path.is_file() or any(
            item.is_file() and re.fullmatch(shade_regex, item.name)
...
def get_palette(input):
    if isinstance(input, list):
        ...
        raise Exception(
            f"Input to get_palette must be a string, Path object, or list of indices, not {input}"
        )
...
def build_palette_ramp(network, palette, ramp_name, ramp_input, input_builder):
    shade_ramp = network.utility("aiRampRgb", ramp_name)
    shade_ramp.attr("type").set(0)
    ramp_input >> shade_ramp.input
    ...
    for shade_index, (facet_image, luminance_value) in enumerate(
        ...
        color_input = input_builder(shade_index, facet_image)

        color_input >> shade_ramp.ramp[start_index].ramp_Color
        color_input >> shade_ramp.ramp[end_index].ramp_Color

    return shade_ramp


def uv_input_builder_builder(network):
    def input_builder(shade_index, shade_image):
        image_texture = network.texture(
            "file", f"shade_{shade_index}", isColorManaged=True
        )
        image_texture.fileTextureName.set(shade_image.image, type="string")
        texture_placement = network.utility(
            "place2dTexture", f"placement_{shade_index}"
        )
        connect_texture_placement(texture_placement, image_texture)
        return image_texture.outColor
