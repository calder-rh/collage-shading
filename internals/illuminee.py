...
class Illuminee(Network):
    ...
    def __init__(self, context, obj):
        ...
        if obj_type == "transform":
            ...
            raise Exception(
                "Invalid illuminee input. Must be a mesh or transform node."
            )
        ...
        for transform in listRelatives(self.control_node, ad=True, type="transform"):
            if (
                transform.hasAttr("used_as_illuminee")
                and transform.used_as_illuminee.get()
            ):
        ...
        is_new = not (
            self.control_node.hasAttr("used_as_illuminee")
            and self.control_node.used_as_illuminee.get()
        )
        if is_new:
            ...
            add_attr(self.control_node, ln="internals", at="compound", nc=7)

            add_attr(
                self.control_node,
                p="internals",
                ln="used_as_illuminee",
                at="bool",
                dv=True,
            )
            add_attr(
            ...
            add_attr(
                self.control_node, p="internals", ln="excluded_lights", at="message"
            )
            ...
            base_lightness = self.add(
                gcn.default_lightness, 0, "base_lightness", return_node=True
            )
            weighted_luminance = self.multiply(
                gcn.luminance, 0, "weighted_luminance", return_node=True
            )
            do_later(
                lambda: self.control_node.lightness_offset >> base_lightness.floatB,
                wait=0.1,
            )
            do_later(
                lambda: self.control_node.contrast >> weighted_luminance.floatB,
                wait=0.1,
            )
            lightness = self.add(
                base_lightness.outFloat, weighted_luminance.outFloat, "lightness"
            )
            lightness >> self.control_node.lightness

            # Calculate the atmosphere blend (0 = fully object, 1 = fully atmosphere)
            illuminee_decomposer = self.utility(
                "decomposeMatrix", "illuminee_decomposer"
            )
            ...
            offset_distance = self.subtract(
                distance.distance, gcn.min_distance, "offset_distance"
            )
            num_half_distances = self.divide(
                offset_distance, gcn.half_distance, "num_half_distances"
            )
            ...
            (
                atmospheric_perspective_amount
                >> self.control_node.internals.atmosphere_blend
            )
            ...
            if not self.control_node.hasAttr("scale_factor", checkShape=False):
                add_attr(self.control_node, ln="scale_factor", min=0, smx=10, dv=1)
                self.load_meshes()
    ...
    def unload_meshes(self):
        for attribute in listConnections(
            self.control_node.lightness, s=False, d=True, p=True
        ):
            gcn.default_lightness >> attribute
        for attribute in listConnections(
            self.control_node.atmosphere_blend, s=False, d=True, p=True
        ):
            attribute.set(0)
    ...
    def link_lights(self):
        default_lights = set(
            sets(lighting_sets.default_lights, union=ls("::default_lights", sets=True))
        )
