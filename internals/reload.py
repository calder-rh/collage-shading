...
def reload():
    illuminees = [
        item
        for item in ls()
        if item.hasAttr("used_as_illuminee") and item.used_as_illuminee.get()
    ]
    default_lights = set(
        sets(SCENE.default_lights, union=ls("::default_lights", sets=True))
    )

    for node in illuminees:
        # the latter is for backwards compatibility
        if node.regular_illuminee.get() or (not node.ground_illuminee.get()):
        ...
        lightlink(
            object=node, light=ls(type="light") + ls(type="aiSkyDomeLight"), b=True
        )
