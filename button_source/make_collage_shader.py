...
import importlib
from internals import network, shading_path, utilities, global_controls, surface_values, coordinate_converter, palettes, illuminee, collage_shader, world_placement, screen_placement, tracking_projection, dialog_with_support
...
def run():
    ...
    dialog_output = fileDialog2(fm=1, cap='Choose palette, facet settings file, or map', okc='Create shader', dir=shading_path())
    if dialog_output is None:
        exit()

    selected_file = Path(dialog_output[0])
    if selected_file.suffix == '.json':
        selected_file_type = SelectedFileType.facet_settings
    elif shading_path('maps') in selected_file.parents:
        selected_file_type = SelectedFileType.map
    else:
        selected_file_type = SelectedFileType.palette
    

    if selected_file_type == SelectedFileType.map:
        ...
        for node in selection:
            if node.type() == 'mesh':
                ...
                continue
            CollageShader({'mesh': format_unique_name(obj)}, obj, map_image_path=map_image_path)
        ...
        for node in selection:
            if node.type() == 'mesh':
                obj = node.getTransform()
            elif node.type() == 'transform' and (shape := node.getShape()) and shape.type() == 'mesh':
                obj = node
            else:
                continue

            if selected_file_type == SelectedFileType.facet_settings:
                with selected_file.open() as file:
                    CollageShader({'mesh': format_unique_name(obj)}, obj, settings=json.load(file))
            else:
                CollageShader({'mesh': format_unique_name(obj)}, obj, palette=selected_file)
